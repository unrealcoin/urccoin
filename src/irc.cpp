...
// Copyright (c) 2011-2012 UnrealCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("UnrealCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #UnrealCoinTEST0\r");
            Send(hSocket, "WHO #UnrealCoinTEST0\r");
        } else {
            // randomly join #UnrealCoin00-#UnrealCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #UnrealCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #UnrealCoin%02d\r", channel_number).c_str());
        }
