...
uint256 hashGenesisBlock("0xddf68bcfaf061786774e7b41d7e6de80c241fafbc20125f93435385dc0e9d96f");
...
const string strMessageMagic = "UnrealCoin Signed Message:\n";
...
{
    // UnrealCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
/* current difficulty formula, dash - DarkGravity v3, written by Evan Duffield - evan@dash.io */
...
{
    // UnrealCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "URC Genesis Block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0402847102a679ad4023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1427466745;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1844622;

        if (fTestNet)
        {
        	block.nTime    = 1388886557;
            block.nNonce   = 387006661;
        }
	    ...
	    assert(block.hashMerkleRoot == uint256("0x779537a5465f15ea522b83f31857653980dccff88dc3a9bcaffa4589439bdf27"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UnrealCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UnrealCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UnrealCoinMiner(CWallet *pwallet)
{
    printf("UnrealCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("UnrealCoin-miner");
    ...
    try { loop {
        ...
        printf("Running UnrealCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UnrealCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UnrealCoinMiner, pwallet));
}
